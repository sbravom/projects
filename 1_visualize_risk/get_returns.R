...
cumulative_results <- function(risk_level, ptiles = c(0.20, 0.5, 0.80), years = 40, nobs = 100000){
  ps <- portstats(risk_level)
  # Generate a matrix of returns
  raw_returns <- matrix(rnorm(n = nobs, mean = ps['er'], sd = ps['vol']) / 100,
                        nrow=years)
  returns_cumul <- apply(raw_returns, 2, function(x) {cumprod(x+1)}) - 1 # add at the end -1 for returns only
  
  # For each year, find the pdf of cumulative returns
  returns_cumul_dens_by_year <- apply(returns_cumul, 1, density)
  names(returns_cumul_dens_by_year) <- paste0("year_", seq(length(returns_cumul_dens_by_year)))
